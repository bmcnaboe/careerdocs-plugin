...
description: Core conventions and the careerdocs CLI shared by the career-documents flows (onboard, update, resume, cover letter). Consult this skill for how the four authorities (qualifications, voice, templates, target role) are modeled and located, how flow skills invoke the careerdocs CLI, the diff-then-approve rule for every profile change, visibility semantics, and where workflow state and generated outputs live. Load it before running any careerdocs command or when a flow skill references a convention it does not restate.
license: MIT
...
1. **Qualifications** — the `CareerProfile`: entities (contact, experience, achievement,
   ...
   **provider** (structured Markdown by default; Basic Memory optional as authoritative).
...
Entry point: `skills/career-documents/scripts/careerdocs.py`. Flow skills call it by
relative path:

```sh
uv run <skill-root>/career-documents/scripts/careerdocs.py <command> --workspace <dir> [--json]
...
- Workflow state: `.career-documents/state/<flow>/<subject>.json` — append-only
...
- `references/schema.md` — the `CareerProfile` model and `career-documents.json` schema.
