---
name: career-onboard
description: Onboard an applicant's existing career materials into one authoritative profile. Use when the applicant wants to import resumes, LinkedIn/network exports, and notes and reconcile them into a single career-documents profile rather than scattered copies. The flow inventories the sources, runs profile import, has the agent extract candidates into candidates.json against the schema, builds a ProfileDiff, asks only the material questions the CLI generates (conflicts, missing dates, undecided visibility), records approval only after an explicit applicant yes, applies the diff, and refreshes the derived export. Resumable — an interrupted run continues from its workflow state without re-asking answered questions.
license: MIT
compatibility: "Python 3.11+; uv recommended. Requires the career-documents core skill and its careerdocs CLI."
metadata:
  version: "0.1.0"
...
Read the core `career-documents` skill first for the conventions this flow obeys
...
1. **Inventory the sources** the applicant points you at (resumes, exports, notes).
2. **`profile import <source>...`** — registers each source with its sha256 and emits text
   blocks (and, for a structured export, draft candidates).
3. **Extract candidates** — read the emitted text blocks and write `candidates.json` (each
...
4. **`profile diff candidates.json --flow onboard --subject <name>`** — merges candidates
   (dedup, precedence, conflicts) into a ProfileDiff and generates the material questions.
5. **Ask only the generated questions** — surface each to the applicant; record answers
...
6. **Turn answers into resolution operations** and re-diff, so the proposed profile
   reflects the applicant's choices.
7. **`profile approve <diff_id>`** — only after an explicit applicant "yes" to the rendered
   diff.
8. **`profile apply <diff_id>`** — writes the authoritative profile atomically and refreshes
   the derived export.
9. **`profile export`** — confirm the derived copy is present and labeled `derived: true`.
