...
def test_link_install_is_byte_identical(tmp_path):
    ...
    target = install.target_base(tmp_path) / "career-documents" / "SKILL.md"
    source = ROOT / "skills" / "career-documents" / "SKILL.md"
    assert target.read_bytes() == source.read_bytes()
    assert (install.target_base(tmp_path) / "career-documents").is_symlink()


def test_copy_install_is_byte_identical_and_real(tmp_path):
    install.install(ROOT, MANIFEST, tmp_path, mode="copy")
    installed = install.target_base(tmp_path) / "career-documents"
    assert not installed.is_symlink()
    assert (installed / "SKILL.md").read_bytes() == (ROOT / "skills" / "career-documents" / "SKILL.md").read_bytes()
...
def test_install_then_uninstall(tmp_path):
    install.install(ROOT, MANIFEST, tmp_path, mode="copy")
    assert (install.target_base(tmp_path) / "career-documents").exists()
    install.uninstall(MANIFEST, tmp_path)
    assert not (install.target_base(tmp_path) / "career-documents").exists()
