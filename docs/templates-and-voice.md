...
- `sections` — a list; each has `id`, `title`, and `units`. Each unit has `text`, `kind`
  (`bullet`, `field`, or `sentence`), and `head` / `tail`, the text split at its first
  tab. An experience unit is `Title, Organization<tab>Mar 2018 – Jun 2021`, so `head` is
  the role and `tail` the dates; other units have an empty `tail`.
